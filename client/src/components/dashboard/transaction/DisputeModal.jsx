...
const DisputeModal = ({ isOpen, onClose, onSubmit, isLoading }) => {
    ...
    const handleSubmit = () => {
        ...
        setDisputeReason(''); // Reset after submit if needed, though usually parent handles closing
    };

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Open Dispute" size="md">
            <div className="space-y-6">
                <div className="bg-amber-50 p-4 rounded-xl border border-amber-100 flex gap-3">
                    ...
                    <p className="text-sm text-amber-800 leading-relaxed">Opening a dispute will pause the transaction and alert our support team. We encourage you to communicate with the counterparty first.</p>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                    <Button variant="secondary" onClick={onClose} className="flex-1 h-12 rounded-xl font-semibold">Cancel</Button>
                    <Button variant="danger" onClick={handleSubmit} isLoading={isLoading} disabled={disputeReason.length < 10} className="flex-1 h-12 rounded-xl font-semibold">Submit Dispute</Button>
                </div>
            </div>
        </Modal>
    );
