...
const register = async (userData, ip, userAgent) => {
  ...
  const accessToken = generateAccessToken(user._id, user.security.tokenVersion, sessionTimeout);
...
const login = async (email, password, mfaToken, ip, userAgent) => {
  ...
  const maxAttempts = settings.maxLoginAttempts || parseInt(process.env.ACCOUNT_LOCKOUT_MAX_ATTEMPTS) || 5;
  const lockoutDuration = settings.lockoutDurationMinutes || parseInt(process.env.ACCOUNT_LOCKOUT_DURATION_MINUTES) || 5;
  ...
  if (!isValidPassword) {
    await user.incrementFailedAttempts(maxAttempts, lockoutDuration);
  ...
  const [device, isNewDevice] = await Promise.all([
    ...
    UserDevice.exists({ user: user._id, ipAddress: ip, userAgent }).then(
      (exists) => !exists,
    ),
  ]);
  ...
  if (isNewDevice) {
    ...
    logger.info("New login device detected", {
      userId: user._id,
      ip,
      deviceInfo,
    });
...
const refreshAccessToken = async (refreshToken, ip, userAgent) => {
  ...
  const setting = await SystemSetting.findOne({ key: "sessionTimeoutMinutes" });
  const sessionTimeout = setting?.value || 15;
  const newAccessToken = generateAccessToken(user._id, user.security.tokenVersion, sessionTimeout);
...
const logoutAll = async (userId, ip) => {
  await Promise.all([
    RefreshToken.revokeAllUserTokens(userId, ip),
    User.findByIdAndUpdate(userId, { $inc: { 'security.tokenVersion': 1 } })
  ]);
...
const deleteAccount = async (userId, ip, userAgent) => {
  ...
  if (activeTransactions) {
    throw new Error(
      "Cannot delete account while you have active transactions. Please complete or cancel them first.",
    );
  }
